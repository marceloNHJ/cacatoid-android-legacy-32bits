...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <!-- Actual refid of the document. If boost.doxygen.refid set then that is the refid.
       Otherwise, it is boost.doxygen.reftitle normalized to an id-compatible string. -->
  <xsl:variable name="boost.doxygen.refid.effective">
    <xsl:choose>
  ...
  <!-- Prefix to add to the Doxygen-generated ids. -->
  <xsl:variable name="doxygen-id-prefix">
    <xsl:text>doxygen.</xsl:text>
    <!-- To disambiguate ids from multiple documents generated by Doxygen
         with SHORT_NAMES=YES, add the document refid to the prefix. -->
    <xsl:if test="string($boost.doxygen.refid.effective) != ''">
      <xsl:value-of select="concat($boost.doxygen.refid.effective, '.')"/>
    </xsl:if>
  </xsl:variable>
  ...
  <xsl:template match="doxygen">
    <library-reference>
      <xsl:if test="string($boost.doxygen.reftitle) != ''">
        <!-- When a reference section has a reftitle, also give it a refid. -->
        <xsl:attribute name="id">
          <xsl:copy-of select="$boost.doxygen.refid.effective"/>
        </xsl:attribute>
